...
- **`UDataAssetPlantPart::AcceptsToolAction(FToolAction)` is the single tool→target gate.** Both `ABasePlant` (harvesting off the growing plant) and `APlantPart` (working a part already on the table) call it from their `CanAcceptToolAction`, so the two moments cannot drift apart. Actor-level state (`HerbStatus`, the harvest-row lookup) stays in the actor; everything data-driven lives on the definition. Do not re-inline the tag checks into a target

### `ToolAction.*` vs `Processing.*` — two families, deliberately
- `ToolAction.*` is a **verb** — "can this tool do this to this thing right now". It is the gate, matched against `UDataAssetPlantPart::AcceptedToolActions`
- `Processing.*` is the **resulting state** — written onto `FItemInstanceData::ProcessingTags` by `UProcessingComponent::BuildProcessedInstance`, read by `UAlchemyCalculationSubsystem::GetProcessingMultiplier` to scale substance yield
- They are not redundant: `ABasePlant::ApplyToolActionResult` deliberately *wipes* `ProcessingTags`, so `ToolAction.Cut` means "harvest a raw part" on a plant and "chop" on a table part. The naming encodes it — imperative (`ToolAction.Crush`) vs past participle (`Processing.Crushed`)
- **A harvest action must keep `ProcessingMethod = null`.** Since `ABasePlant` now honours `AllowedProcessingTags` too, attaching a processing method to a harvest action would demand a tag the harvest then discards. If table-cutting should produce `Processing.Cut`, add a *separately tagged* second action (`ToolAction.Chop`) — `TryStartToolAction` runs the first accepted action in array order, so two entries sharing one `ActionTag` make the second unreachable
- Nothing in C++ maps one family onto the other. The only link is authored: an `FToolAction` pairs an `ActionTag` with a `ProcessingMethod` asset that independently holds a `ProcessingTag`

### Data asset validation
`UDataAssetPlantPart` and `UToolItemDefinition` override `IsDataValid` (editor-only, `Misc/DataValidation.h`, already in `CoreUObject` — no `Build.cs` change). They exist because **every misconfiguration in this system fails silently**: `TryStartToolAction` skips unusable actions with a bare `continue`, and the caller falls back to dragging, so a dead tool and a refused target look identical. Right-click → *Validate Asset* catches the empty-`Actions` tool, the empty-`AllowedProcessingTags` part (an empty container allows **nothing**), wrong-family tags, actions with no minigame widget, and duplicate `ActionTag`s. All findings are warnings, not errors — half-authored assets are a normal mid-session state. Add checks here rather than adding runtime logging
